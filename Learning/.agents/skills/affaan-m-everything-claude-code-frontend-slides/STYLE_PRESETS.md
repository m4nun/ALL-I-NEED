...
| 元素 | 最低对比度 | 常见失败 |
|---|---|---|
| 正文文字 | 4.5:1 | 使用 `#9ca3af` 等中灰代替浅灰 `#f3f4f6` |
| 强调色文字 | 4.5:1 | 深蓝 `#0047AB` 在黑色上仅有 ~2.4:1，改用 `#2563EB` 或更亮的蓝色 |
| 边框/装饰线 | 3.0:1 | 同上的深蓝色在深色背景上对比不足 |
| 标签 (tag) | 4.5:1 | 深色背景上的标签文字必须使用更亮的强调色 |
| 导航圆点（非激活） | 3.0:1 | `rgba(255,255,255,0.3)` 不够，建议 `0.45` 以上 |
| 键盘提示 | 4.5:1 | 固定定位的文字在白色幻灯片上会消失，需添加深色背景药丸 |

### 白色幻灯片（背景白色）

| 元素 | 最低对比度 | 常见失败 |
|---|---|---|
| 正文文字 | 4.5:1 | 使用 `#9ca3af` 等浅灰在白色上仅有 ~2.5:1 |
| 导航圆点（非激活） | 3.0:1 | `rgba(0,0,0,0.2)` 不够，建议 `0.5` 以上 |
| 辅助文字 | 4.5:1 | 避免在白色上使用 `#9ca3af`，改用 `#4b5563` 或更深 |

### 修复策略

1. **使用 CSS 变量分层**：定义 `--cobalt`（深色，用于白色背景）和 `--cobalt-light`（较亮，用于深色背景）
2. **为深色幻灯片覆盖标签颜色**：`.slide-dark .tag { color: var(--cobalt-light); border-color: var(--cobalt-light); }`
3. **为固定元素添加背景**：键盘提示、导航圆点等固定元素应添加半透明背景药丸以保证跨幻灯片可读性
4. **使用脚本验证**：运行 `python3 contrast-check.py presentation.html` 自动检测所有失败的对比度组合
